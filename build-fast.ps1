    ...
    if (-not (Test-Path ".env.production")) {
        if (Test-Path ".env.production.example") {
            try {
                ...
                Handle-Error "Failed to copy environment file"
            }
        } else {
            Write-ColorOutput "$($Symbols.Warning) .env.production.example not found, skipping..." "Yellow"
        }
    }
    Start-Sleep -Milliseconds 500
...
function Show-FinalStatus {
    ...
    Write-Host ""
    Write-ColorOutput "$($Symbols.Rocket) Deployment completed in record time! $($Symbols.Rocket)" "Green"
    ...
    Write-ColorOutput "4. Run security scan: docker-compose -f docker-compose.production.optimized.yml exec app php spark security:scan" "Yellow"
}
